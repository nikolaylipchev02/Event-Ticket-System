...
public sealed class EventApiClient(HttpClient httpClient) : IEventApiClient
{
    ...
    public async Task UpdateEventAsync(Guid id, UpdateEventRequest request, CancellationToken cancellationToken = default)
    {
        HttpResponseMessage response = await httpClient.PatchAsJsonAsync($"api/events/{id}", request, cancellationToken);
        response.EnsureSuccessStatusCode();
    }
