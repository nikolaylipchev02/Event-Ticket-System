...
public sealed class AuthController(IUserApiClient userApiClient, ILogger<AuthController> logger) : ControllerBase {
    ...
    public async Task<IActionResult> Login([FromBody] AuthLoginRequest request, CancellationToken cancellationToken) {
        try {
            ...
            return StatusCode(StatusCodes.Status502BadGateway,
                    new { message = "The user service is not reachable right now." });
        }
    }
    ...
    void SetAuthCookie(string accessToken) {
    ...
    static DateTimeOffset ResolveTokenExpiration(string accessToken) {
        ...
        try {
            ...
            if (payload.RootElement.TryGetProperty("exp", out JsonElement expirationElement) &&
                expirationElement.TryGetInt64(out long expirationSeconds)) {
